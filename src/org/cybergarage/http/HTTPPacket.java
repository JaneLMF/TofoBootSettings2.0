...
public class HTTPPacket {
	...
	protected boolean set(InputStream in, boolean onlyHeaders) {
		try {
			...
			if (statCode == HTTPStatus.CONTINUE) {
				...
				// iis sends 100 code response and a 200 code response in the
				// same
			...
			if (isChunkedRequest == true) {
				try {
					...
					// (07/07/05)
					// contentLen = Long.parseLong(new
					// String(chunkSizeLine.getBytes(), 0,
			...
			while (0 < contentLen) {
				...
				byte readBuf[] = new byte[(int) (contentLen > chunkSize ? chunkSize
						: contentLen)];
				...
				if (isChunkedRequest == true) {
					...
					try {
						String chunkSizeLine = readLine(reader);
						// Thanks for Lee Peik Feng
						// <pflee@users.sourceforge.net> (07/07/05)
						contentLen = Long.parseLong(
								new String(chunkSizeLine.getBytes(), 0,
										chunkSizeLine.length() - 2), 16);
					} catch (Exception e) {
						contentLen = 0;
					}
	...
	/**
	 * 根据名字获取httpHeaderList的元素，如果与
	 * httpHeaderList元素中的HTTPHeader的name字段相同,不区分大小写则返回HTTPHeader 否则返回false
	 */
	...
	/**
	 * 如果httpHeaderList中有HTTPHeader的名字与name相同则设置value的值 否则创建一个新的HTTPHeader
	 * 添加到httpHeaderList中
	 * 
	 * @param name
	 *            获取HTTPHeader，或设置HTTPHeader的name字段的值
	 * @param value
	 *            设置HTTPHeader 的值
	 * 
	 **/
	...
	/** 获取头的值，返回是没有前缀和后缀的 */
	public String getStringHeaderValue(String name, String startWidth,
			String endWidth) {
	...
	/**
	 * 设置 content的值 如果updateWithContentLength 为true添加Content-Length
	 * 消息头到httpHeaderList集合中
	 */
	...
	/**
	 * 调用setContent(byte data[], boolean updateWithContentLength) data
	 * 为data.getBytes() updateWithContentLength为updateWithContentLength
	 */
	...
	/**
	 * 设置 Content-Length 头的值 调用 setContent(String data, boolean
	 * updateWithContentLength) updateWithContentLength 为true
	 */
	...
	// //////////////////////////////////////////////
	// Contents (SmbRandomAccessFile)
	// //////////////////////////////////////////////

//	/** 内容的输入流 */
//	private SmbRandomAccessFile smbRdaFile = null;
//
//	/** 设置内容的输入流 InputStream contentInput */
//	public void setContentInputSmbRdaFile(SmbRandomAccessFile in) {
//		smbRdaFile = in;
//	}
//
//	/** 获取 内容的输入流 InputStream contentInput */
//	public SmbRandomAccessFile getContentInputSmbRdaFile() {
//		return smbRdaFile;
//	}
//
//	/** 判断 contentInput 是否为null，不为null返回true，为null返回false */
//	public boolean hasContentInputSmbRadFile() {
//		return (smbRdaFile != null) ? true : false;
//	}
	...
	/**
	 * 设置HTTPHeader 的name为Connection value为value的值 设置是否保存连接
	 */
	...
	/**
	 * 设置HOST头的值 设置HTTPHeader的name和value的值 name 的值为HOST ，value的值为 hostAddr + ":"
	...
	/**
	 * 如果没有 Transfer-Encoding 消息头返回false 如果有获取Transfer-Encoding
	 * 消息头对象的值，如果与Chunked相同,不区分大小写，就返回true，否则返回false
	 */
