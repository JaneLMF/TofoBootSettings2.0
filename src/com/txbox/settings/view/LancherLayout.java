...
public class LancherLayout extends LinearLayout implements ShadowCallBack, View.OnClickListener, View.OnFocusChangeListener {
	...
	public LancherLayout(Context paramContext) {
		...
		addView(LayoutInflater.from(this.context).inflate(R.layout.layout_launcher, null));
		initView();
	}
...
//				shadowBackgrounds[paramInt].setVisibility(View.VISIBLE);
	...
	public void initView() {
		...
		this.imageViews[0] = ((ImageView) findViewById(R.id.QQ));
		this.imageViews[1] = ((ImageView) findViewById(R.id.weChat));
		this.imageViews[2] = ((ImageView) findViewById(R.id.ignore));
	...
	public void onFocusChange(View paramView, boolean paramBoolean) {
		...
		case R.id.QQ:
			...
			x = DensityUtil.dip2px(context, 300);
			y = DensityUtil.dip2px(context, 140);
			// 此处可封装出数学算法，计算偏移量的x,y值。我懒得算了，直接设置了
			break;
		case R.id.weChat:
			i = 1;
			x = DensityUtil.dip2px(context, 530);
			y = DensityUtil.dip2px(context, 140);
			break;
		case R.id.ignore:
			i = 2;
			x = DensityUtil.dip2px(context, 760);
			y = DensityUtil.dip2px(context, 140);
			break;
		}
		if (paramBoolean) {
